package log

import (
	"io/ioutil"
	"os"
	"testing"

	api "github.com/abdelwhab-1/proglog/api/v1"
	"github.com/stretchr/testify/require"
	"google.golang.org/protobuf/proto"
)

func TestLog(t *testing.T) {

	for scenario, fn := range map[string]func(t *testing.T, log *log){
		"testReadAppend":              testReadAppend,
		"offset out of range error":   testOutOfRangeErr,
		"init with existing segments": testInitExisting,
		"reader":                      testReader,
		"truncate":                    testTruncate,
	} {
		t.Run(scenario, func(t *testing.T) {
			dir, err := ioutil.TempDir("", "log_test")
			require.NoError(t, err)
			defer os.RemoveAll(dir)
			c := Config{}
			c.Sagment.MaxStoreBytes = 32
			log, err := NewLog(dir, c)
			require.NoError(t, err)
			fn(t, log)
		})
	}
}

func testReadAppend(t *testing.T, log *log) {
	record := &api.Record{Value: []byte("hello world")}
	off, err := log.Append(record)
	require.NoError(t, err)
	require.Equal(t, uint64(0), off)

	red, err := log.Read(0)
	require.NoError(t, err)
	require.Equal(t, record.Value, red.Value)
	require.Equal(t, record.Offset, red.Offset)
}

func testOutOfRangeErr(t *testing.T, log *log) {
	red, err := log.Read(1)
	require.Nil(t, red)
	apiErr := err.(api.ErrOffsetOutOfRange)
	require.Equal(t, uint64(1), apiErr.OffSet)

}

func testInitExisting(t *testing.T, log *log) {
	append := &api.Record{Value: []byte("hello world")}
	for i := 0; i < 3; i++ {
		_, err := log.Append(append)
		require.NoError(t, err)
	}
	err := log.Close()
	require.NoError(t, err)
	off, err := log.LowestOffset()
	require.NoError(t, err)
	require.Equal(t, uint64(0), off)

	off, err = log.HighestOffset()
	require.NoError(t, err)
	require.Equal(t, uint64(2), off)

	nLog, err := NewLog(log.Dir, log.Config)
	require.NoError(t, err)

	off, err = nLog.LowestOffset()
	require.NoError(t, err)
	require.Equal(t, uint64(0), off)

	off, err = nLog.HighestOffset()
	require.NoError(t, err)
	require.Equal(t, uint64(2), off)
}

func testReader(t *testing.T, log *log) {
	append := &api.Record{Value: []byte("hello world")}
	off, err := log.Append(append)
	require.NoError(t, err)
	require.Equal(t, uint64(0), off)

	reader := log.Reader()
	val, err := ioutil.ReadAll(reader)
	require.NoError(t, err)
	red := &api.Record{}
	err = proto.Unmarshal(val[lenWidth:], red)
	require.NoError(t, err)
	require.Equal(t, append.Value, red.Value)
}

func testTruncate(t *testing.T, log *log) {
	append := &api.Record{Value: []byte("hello world")}
	for i := 0; i < 3; i++ {
		_, err := log.Append(append)
		require.NoError(t, err)
	}
	err := log.Truncate(1)

	require.NoError(t, err)
	red, err := log.Read(0)
	require.Error(t, err)
	require.Nil(t, red)
}
